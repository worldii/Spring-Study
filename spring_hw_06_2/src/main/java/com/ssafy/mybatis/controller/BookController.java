...
public class BookController {
	...
	public String doLogin(@ModelAttribute User user, HttpSession session, Model model) {
		User tempUser = userService.join(user);
		if (tempUser != null)
		{
			user.setName("김싸피");
	...
	@GetMapping("/regist_result/{isbn}")
	public String showRegistResult (@PathVariable String isbn, Model model)
	{
		Book select = bookService.select(isbn);
	 ...
	 */
	@PostMapping("/regist")
	public String doRegist(@ModelAttribute Book book, @RequestPart(required = false) MultipartFile file, RedirectAttributes redirectAttributes, Model model)
		...
		log.info("regist Book {}", result);
		if (result != -1)
		{
			// PRG
			return "redirect:/regist_result/"+book.getIsbn();
		}
		else {
